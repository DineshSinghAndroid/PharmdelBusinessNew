
/// Common string's
const String kTest = "Test";
const String kRetry = "Retry";
const String kError = "Error";
const String kNoInternet = "NoInternet";
const String kForgotPassword = 'Forgot Password';
const String kHowToGuide= "How to guide";
const String kByContOur = "By continuing, you agree to our ";
const String kTermsOfService = 'Terms of Use';
const String kPrivacyPolicy = 'Privacy Policy';
const String kCustomerList = 'Customer List';
const String kAddNewCustomer = 'Add New Customer';
const String kMapView = "Map View";
const String kCloseTote = "Close Tote";
const String kPharmdel = "Pharmdel";
const String kCreate = "Create";
const String kUserNotRegisterContinueToCreate = "User not registered, Continue to create a new user";



/// Logo
const String kSplashLogo = "assets/logo/logo.png";


///Image PNG
const String strImg_tick = 'assets/images/tick.png';

const String strIMG_HomeBg = "assets/images/bottom_bg.png";
const String strIMG_SpeedoMeter = "assets/images/speedometer.png";
const String strIMG_AltAdd = "assets/images/alt-add.png";
const String strIMG_DelTruck = "assets/images/delivery_truck.png";
const String strIMG_Sad = "assets/images/sad.png";
const String strIMG_location = "assets/images/location_top.png";
const String strIMG_Map = "assets/images/google-maps-icon.png";
const String strImgRightClick = "assets/images/tick.png";
const String strIMG_DelTruck2 = "assets/images/delivery_icon.png";
const String strIMG_Location = "assets/images/location_icon.png";
const String strIMG_QrCode = "assets/images/qr_code.png";
const String strIMG_Fridge = "assets/images/fridge.png";
const String strImgFridge = "assets/images/fridge.png";
const String strImgAutomaticIcon = "assets/images/automatic_icon.png";
const String strImgAltAdd = "assets/images/alt-add.png";
const String strImgSend = "assets/images/send.png";
const String strImgDelTruck = "assets/images/delivery_truck.png";
const String strImgHomeBg = "assets/images/bottom_bg.png";
const String strImgHomeIcon = "assets/images/home_icon.png";
const String strImgQrCode = "assets/images/qr_code.png";
const String strImgMap = "assets/images/google-maps-icon.png";
const String strImgSpeedometer = "assets/images/speedometer.png";
const String strGifForRouteStart = "assets/images/route_loading.gif";
const String strImgSad = "assets/images/sad.png";
const String strImgCalendar = "assets/images/calendar.png";




///GIF
const String strIMG_NewGIF = "assets/images/new.gif";
const String strImgNewGif = "assets/images/new.gif";
const String strGIF_processing = "assets/images/processing.gif";
const String strGIF_LOADING = "assets/images/loading.gif";

/// Toast Msg
const String kStoragePermissionToast = "msg...";
const String kChangeRouteMsg = "You are already on a route, you can't change before completed.";
const String kDontHaveroute = "You don't have any route. Try again after and refresh now.";
const String kThisOrderAlreadyCompleted = "This order already completed.";
const String kPleaseOnLocaionPermission = "Please go to mobile setting and on location permission";
const String kThisOrderAlreadyExits = "This order already exits.";
const String kThisOrderAlreadyOutForDelivery = "This order not for out for delivery.";


///DashBoard Screen
const String kOrderID = "Order ID";
const String kOrderIdNotFound = "Order ID not found";
const String kNursingHomeID = "Nursing Home ID";
const String kOrderDetailNotFound = "Order detail not found!";
const String kBulkScan = "Bulk Scan";
const String kDeliverNow = "Deliver now";
const String kRescheduleDelivery = "Reschedule Delivery";
const String kWouldYouLikeToReschedule = "Would you like to reschedule";
const String kSelectRescheduleDate = "Select Reschedule Date";
const String kAreYouOkayToProceed = "Are you okay to proceed ?";
const String kManual = "Manual";
const String kUpdateCustomerMobileNumber = "Update customer mobile number";
const String kDeliveryTo = "Delivery to";
const String kDeliveryRemark = "Delivery remark";
const String kRoute = "Route";
const String kRequested = "Requested";
const String kOptimizingRoute = "Optimizing Route";
const String kReady = "Ready";
const String kDeliveryNote = "Delivery Note";
const String kDeliveryCharge = "Delivery Charge";
const String kRxCharge = "Rx Charge";
const String kExistingDeliveryNote = "Existing Delivery Note";
const String kTotal = "Total";
const String kOther = "Other";
const String kPickedUp = "Picked Up";
const String kUnpick = "Unpick";
const String kDelivered = "Delivered";
const String kReadDeliveryNotes = "Read Delivery Notes";
const String kReadCustomerNotes = "Read Customer Note";
const String kPaymentExemption = "Payment Exemption";
const String kExempt = "Exempt";
const String kBookDelivery = "BookDelivery";
const String kDeliveredTo = "Delivery to";
const String kPickedUpDelivery = "pickedup";
const String kPickedUp2 = "PickedUp";
const String kOthers = "Others";
const String kDeliveredRemark = "Delivery remark";
const String kDelivery = "Delivery";
const String kFailed = "Failed";
const String kOnTheWay = "On the way";
const String kStartRoute = "START ROUTE";
const String kScanRx = "Scan Rx";
const String kScanQr = "Scan QR";
const String kSelectRoute = "Select Route";
const String kSelectRouteFirst = "Select Route First!!";
const String kSelectPharmacyFirst = "Select Pharmacy";
const String kOrderNotFound = "Order Not Found!";
const String kSelectPhar = 'Select Pharmacy';
const String kParcelLocation = 'Parcel Location';
const String kSelectParcelLocation = 'Select Parcel Location';
const String kAddressPara = '17 Fairwood Drive port Talbot West Glamorgan SA128NS';
const String kStatusOutForDelivery = "OutForDelivery";
///pharmacy dashboard
const String strImg_PharmacyBg = "assets/images/pharmacyHomeBg.png";


///Drawer
const String kLunchMode = "LUNCH MODE";
const String kOff = "OFF";
const String kSelectTote = "Select Tote";
const String kSelectDeliveryCharge = "Select Delivery Charge";
const String kSelectExempt = "Select Exempt";
const String kStartLunch = "Click to start lunch time";
const String kON = "ON";
const String kPersonalInfo = 'Personal Info';
const String kContactNumber = 'Contact Number';
const String kemail = "Email";
const String kaddress = 'Address';
const String kChangePin = 'Change Pin';
const String kCreatePatient = 'Create patient';
const String kAppVersion = 'App Version:';
const String kHowToOperate = 'How to operate';
const String kEnterMiles = 'Enter miles';
const String kUpdateAddress = 'Update Address';
const String kVirUpload = 'Upload VIR ';
const String klogout = 'Logout';
const String kFridge = 'Fridge';
const String kSearchMedicine = 'Search Medicine';
const String kPackSize = 'Pack Size';
const String kQuantity = 'Quantity';
const String kDays = 'Days';


///Notificaiton Screen
const String kMyNotifiaction = 'My Notification';
const String kReceived = 'Received';
const String kSent = 'Sent';
const String kCreateNotification = 'Create Notification';
const String kNotificationName = 'Notification Name:';
const String kPharmacyStaffString = "Pharmacy Staff";
const String kPharmacyStaff = "Pharmacy Staff:";
const String kSelectPharStaff = 'Select Pharmacy Staff';
const String kMessage = 'Message';
const String kTrue = 'True';

///Create Patient Screen
const String kSelectTitle = "Select Title";
const String kSelectGender = "Select Gender";
const String kFirstName = "First name";
const String kMiddleName = "Middle name (optional)";
const String kLastName = "Last name";
const String kStartTypingAddress = "Start typing address";
const String kEnterYourAddress = "Enter your address";
const String kEnterYourCorrectAddress = "Enter your correct address";
const String kMobileNumber = "Mobile number";
const String kMobileNumberOptional = "Mobile number(Optional)";
const String kEmailOptional = "Email (optional)";
const String kNHSNumber = "NHS number (optional)";
const String kSearchAddress = "Search Address";
const String kSaveAndCreate = "Save and Create";


///Pop-Up
const String kTakeSpdMtrPic = "take speedometer picture";
const String kPlsEntStrmiles = 'Please enter start miles';
const String kUploadingMsg = "Please wait\nWe are updating your offline deliveries.";
const String kuploadingMsg1 = "Please wait\nWe are updating deliveries and ending route.";
const String kInternetNotAvailable = "Seems like you have weak or no internet.\nPlease connect to internet first!";
const String kPleaseCompleteManually = "Please complete manually";
const String kStartroute = "Are you sure, you want to start route?";
const String kAlert = "Alert...";
const String kOkay = "Okay";
const String kWrongBag = "Wrong bag";
const String kRouteEnd = "Your route has been ended";
const String kYouHaveEnteredStartAndEndMiles = "You have entered start and end miles";
const String kOptimizingRouteMsg = "Pharmacy optimizing the route, please wait it might take some time";
const String kSystemProblemMsg = "There was a problem with the system, please login again";
const String kCheckAgain = "Check Again";
const String kEndRoute = "End Route";
const String kBulkDrop = "Bulk Drop";
const String kComplete = "Complete";
const String kMakeNext = "Make Next";
const String kArrived = "Arrived";
const String kImageOptional = "Image (Optional)";
const String kCustomerSignature = "Customer Signature";
const String kNo = "No";
const String kPaid = "Paid";
const String kClear = "Clear";
const String kEndRouteWarning = "Are you sure,you want to end Route?";
const String kSelectRouteAgain = "Select route and try again!";
const String kPleaseEnterValidDay = "Please enter a Valid Day";
const String kPleaseEnterValueGreaterThanEqualTo1 = "Please enter a value greater than or equal to 1";
const String kAreYouSureWantToStart = "Are you sure, you want to start route?";
const String kReOptimiseRemainingStops = "Re optimise remaining stops";
const String kReOptimiseStops = "Re optimise stops";
const String kReOptimisePopUpSubTitle = "We recommend Re - optimising the remaining stops to make sure you're still on the best route.";
const String kPharmacyAlreadyMakeThisRouteSubTitle = "Pharmacy already make this route, do you want to re-route it.";

const String kOdrCrtdSucc = 'Order Created Successfully';


///Update Address
const String kAddress1Line = "Address Line 1";
const String kAddress2Line = "Address Line 2";
const String kAddressLine1 = "Address Line 1:*";
const String kAddressLine2 = "Address Line 2:";
const String kTownName = "Town Name";
const String kTown = "Town";
const String kPostCode = "Postal Code";
const String kPostalCode = "Postal Code:*";
const String kPharmacy = "Pharmacy";
const String kStartRouteFrom = 'Start Route From';
const String kEndRouteFrom = 'End Route From';
const String kCurrentLocation = 'Current Location';
const String kHomeLocation = 'Home Location';
const String kNoAddressProfile = 'No address in profile ';
const String kDriverNotAvailableSelectedRoute = 'Driver not available on selected route!';


///Lunch Break
const String kYouAreOnBreak = 'You are on Break';
const String kEndBreak = 'End Break';
const String kEndBreakMsg = "(Click 'End Break' button to get back to work)";

///Lottie files links
const String strLottieLunchBreak = "https://assets3.lottiefiles.com/packages/lf20_1fggr4vt.json";


///Lottie files links
const String kPleaseScanOrderNow = "Please scan order now...";


///Search Patient Screen
const String kNoPatAvl = 'No Patient Available';
const String kSearchPat = "Search Patient....";
const String kCstmList = "Customer List";
const String kSelCustomer = "Select Customer";
const String kDOB = "Date of Birth : ";


///Update Status Screen
const String kPatientDetails = "Patient Details";
const String kUpdateStatus = "Update Status";

/// Instructions screen
const String kPress = "Press";
const String kStartBulkDrop = "Start Bulk Drop";
const String kInstructionTitle = "Instructions to Bulk Drop";
const String kInstructionsDes = "Please correlate the package when scanning.";


const List<String> kInstructionsTitle = ["Scan the package you wish to deliver", " to add multiple orders", "Complete Bulk Drop "];
const List<String> kInstructionsContent = ["Scan the package barcode", "Scan barcode and add multiple orders", "Complete the multiple order at a place"];



 ///pharmacy track order screen

///Pharmacy HomeScreen
const String kDeliveries = "Deliveries";
const String kTrackOrder = "Track Order";
const String kScanAndBook = "Scan & Book";
const String kNursHomeBoxBook = "Nursing Home Box Booking";

///pharmacy track order screen
 /// TextField Validation
  const String kSelectDriver = "Select Driver";
 /// TextField Validation
 const String kEnterEmail = "Enter email";
const String kEnterPassword = "Enter password";
const String kEnterYourEmailID = "Enter your mail id";
const String kSubmit = "Submit";
const String kCancel = "Cancel";
const String kAddNow = "Add Now";


/// Other
const String kSelectDeliveryStatusToastString = "Select a delivery status";
const String kTypeFailedRemarkToastString = "Type failed remark";
const String kDeliveryIsNotEmptyToastString = "Delivery to isn't empty";
const String kCheckDeliveryNoteToastString = "Check delivery note";
const String kCheckFridgeToastString = "Check Fridge";
const String kCheckControlledDrugsToastString = "Check Controlled Drugs";
const String kCheckCustomerNoteToastString = "Check customer note";
const String networkToastString = "Please check network connection and try again!";
const String kTouchAndFaceNtAct  = "touch or Face Id is not activated on your device!";
const String kUserLastTime = "lastTime";
const String kName = "Name";
const String kDateOfBirth = "DOB : ";
const String kEmailId = "Email : ";
const String kTotalMiles = "Total Miles";

const String kChooseDeliveryDate = "Choose delivery date";
const String kEnterYourRemarkHere =  "Enter your remark's here";




const String kSplashTitle = "Pharmdel";
const String kLogin = "Login";
const String kEmail = "Email";
const String kPassword = "Password";
const String kRememberMe = "Remember Me";
const String kSkip = "Skip";
const String kSave = "Save";
const String kBack = "Back";
const String kEmptyData = "Empty";
const String kFilters = "Filters";
const String kSearchArea = "Search area";
const String kSearch = "Search...";
const String kDistance = "Distance type";
const String kRange = "Range";
const String kContinue = "Continue";
const String kRenewSub = "Renew Subscription";
const String kApply = "Apply";
const String kReset = "Reset";
const String kGoBack = "Go Back";
const String kSuccess = "Success";
const String kProfile = "Profile";
const String kCompleteBookingTitle = "Enter student details";
const String kPrice = "Price : ";
const String kBookingNumber = "Booking number : ";
const String fPrice = "Price";
const String kAddReview = "Add Review";
const String kRenewSubscription = "Renew Subscription";
const String kTimeSlot = "Time Slot : ";
const String kTimeLeft = "Time Left";
const String kUpdateMobileNumber = "Update Mobile number";
const String kNewDeliveryNote = "New Delivery Note";
const String kExistingNote = "Existing Note";
const String kExistingNoteWithDoted = "Existing Note:";
const String kUpdate = "Update";
const String kCompleteBooking = "Complete Booking";
const String kCompleteOrder = "Complete Order";
const String kGallery = "Gallery";
const String kCamera = "Camera";
const String kReview = "Review";
const String kCdSignIsMandatory = "Cd sign is mandatory";
const String kPaymentType = "Payment Type";
const String kAmount = "Amount";
const String kPayment = "Payment";
const String kStar = "Star : ";
const String kComment = "Comment...";
const String kNotifications = "Notifications";
const String kMyBooking  = "My Booking";
const String kMyQRCode  = "My QR Code";
const String kMyDetails  = "My Details";
const String kMyQrCode  = "My Qr Code";
const String kOk  = "OK";
const String kPleaseSelectPharmacy = "Please Select Pharmacy!!";
const String kPleaseSelectRoute = "Please Select Route!!";
const String kYes  = "Yes";
const String kDone  = "Done";
const String kConfirm  = "Confirm";
const String kNotAuthenticated  = "Not Authenticated";
const String kAuthenticatedDes  = "Someone trying to login in another device so you logout form here!";
const String kQueryDes  = "Thank you for submitting your request we will get in touch with you shortly.";
const String kAreYouSureWantToLogout  = "Are you sure want to logout!";
const String kAreYouSureToEndBreak  = "Are you sure to end break?";
const String kApplyCoupon  = "Apply Coupon";
const String kUserName = "name";
const String kCompleted = "Completed";
const String kQuickPin = "quickPin";
const String kEnterSecurePin = "Enter Secure Pin";
const String kForgotMPIN = "Forgot M-Pin?";
const String kusediffAcc = "Use different Account?";
const String kFormatNotCorrect = "Format not correct!";
const String kFailedToGetPlatformVersion = "Failed to get platform version.";
const String kUSERTYPE = "userType"; //Branch Admin + OR +


///Png Define

const String strimg_logo = 'assets/logo/logo.png';


// Validation string
const String kEnterName = "Enter your name";
const String kEnterAadhaar = "Enter your aadhaar number";
const String kEnterValidAadhaar = "Enter valid aadhaar number";
const String kEnterCity = "Enter your city";
const String kEnterMobileNo = "Enter mobile number";
const String kEnterValidMobileNo = "Enter 10 digit Mobile Number";
const String kEnterDigPin = "Enter 4 digit pin";
const String kEnterValidPin = "Enter valid Pin";
const String kEnterValidEmail = "Enter valid email";
const String kEnterAddress = "Enter address";

// Login
const String kLoginTitle = "Welcome";
const String kLoginBtnTitle = "Login";
const String kLoginNotMember  = "Not a member? ";
const String kLoginSignUpNow  = "SignUp now";
const String kLoginRegisterNow  = "Register now";
const String kLoginMob = "Mobile Number";
const String kAadhaar = "Aadhaar Number";
const String kCity = "City";
const String kCheckTerm = "Please check term and conditions";

const String kChooseVehicle = "Choose Vehicle";
const String kPleaseSelectVehicle = "Please select vehicle";
const String kEnterMilesAndUploadPicture = "Enter miles and upload picture";
const String kPleaseEnterStartMiles = "Please enter start miles";
const String kClickImage = "Click Image";


// SignUp
const String kSignUpTitle = "SignUp";
const String kRegisterTitle = "Register";
const String kSignUpName = "Name";
const String kSignUpIAgree  = "I Agree ";
const String kSignUpTNC  = "T&C";
const String kSignUpAlready = "Already have an account? ";

// Verify otp
const String kVerifyOtpTitle = "Verify Mobile OTP";
const String kVerifyOtpDoNotCode = "Didn't you receive any code ?";
const String kVerifyOtpResend  = "Resend New CODE";
const String kVerifyOtpOtpReceive  = "Resend OTP in ";
const String kSeconds = " seconds";

// Dashboard
const String kHome = "Home";
const String kHistory = "History";


// Query
const String kAskQuery = "Ask Query";
const String kContactInfo = "Contact Info :";

// Profile
const String kEditProfile = "Edit Profile";
const String kHelpAndSupport = "Help & Support";
const String kPrivacyStatement = "Privacy Statement";
const String kTermsAndConditions = "Terms and Conditions";
const String kShareApp = "Share App";
const String kLogOut = "Log Out";


//Home
const String kOpenHour = "Open hour : ";
const String kFacility = "Facility : ";
const String kBookNow = "Book now";
const String kInvoice = "Invoice";
const String kViewDetail = "View Detail";

//Search
const String kSearchLibraries = "Search Libraries";

//My Qr code

// Seat arrangement
const String kBookingDate = "Booking Date : ";
const String kWhereDoYouSit = "Where do you want to sit?";
const String kUnavailable = "Unavailable";
const String kAvailable = "Available";
const String kSelected = "Selected";
const String kSeatArrangement = "Seat Arrangement";
const String kSeat = "Seat";

// Details
const String kDetails = "Details";
const String kDirection = "Direction";
const String kFacilities = "Facilities";
const String kSubscriptionType = "Subscription Type";
const String kMore = "More...";
const String kBagSize = "Bag Size";

// Schedule screen
const String kAddress = "Address";
const String kNotFound = "Not Found";
const String kSchedule = "Schedule";
const String kProceedToBookNow = "Proceed to Book Now";



// Intro
const String introTitle = "Heading";
const String introDescription = "Lorem ipsum dolor sit amet, consetetur sadipsing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna";
const String introTitle2 = "Heading2";
const String introDescription2 = "Lorem ipsum dolor sit amet, consetetur sadipsing elitr, sed diam nonumy eirmod tempor invidunt ut labore et dolore magna";

const String appName = "ApnaSlot";
const String networkErrorText = "Please check network connection and try again!";
const String errorText = "Something went wrong";



// Toast string
const String kSetupPinNotMatchToastString = "Old secure pin did not matched, Please enter correct old secure pin";
const String kQueryToastString = "Please mention your query";
const String kSelectExemptions = "Please select any one exemptions";
const String kPleaseChooseVehicleEnterStartMiles = "Please choose vehicle or Enter Start Miles";
const String kTakeSpeedometerPicture = "Take Speedometer Picture";
const String kFilterToastString = "Please select any option";
const String kSelectOrderToastString = "Please select any order";
const String kReviewToastString = "Please enter your comment";
const String kChoosePlanToastString = "Please choose plan";
const String kNoAnyPlanToastString = "Not available plan";
const String kChooseYourSheet = "Choose your seat";
const String kAlreadyBookedSeat = "This seat already booked";
const String kSeatNotAvailable = "This seat not available";
const String kUpdatePictureToastMsg = "Please update your picture";
const String kSecurePinNotMatch = "Secure pin does not match";
const String kOldPinNotMatch = "'Old pin not correct'";
const String kPlsEntDigPin = "Please enter digit Pin'";
const String kPinDidNotMatch = "''Pin did not match''";
const String kInternetNotAvailable2 = "Something went wrong, Failed connection with server.";
const String kPinDoesNotMatched = "Pin did not match";
const String kChangeQuickAccessPin = "Change Quick Access Pin";
const String kSetupQuickAccessPin = "Setup Quick Access Pin";

const String kChooseNursingHome = "Choose nursing home";
const String kChooseRouteFirst = "Choose route first";


const String kEnterValidMPin = "Enter 4 digit pin";
const String kConfirmPinNotMatch = "Confirm pin not matched";
const String kEnterOldMPin = "Enter Old Pin";
const String kEnterCorrectOldMPin = "Enter correct old pin";
const String kEnterMPin = "Enter Secure Pin";
const String kEnterNewMPin = "Enter New Secure Pin";
const String kEnterValidOldMPin = "Enter 4 digit pin";
const String kEnterValidNewMPin = "Enter 4 digit new pin";
const String kEnterValidConfirmMPin = "Enter 4 digit confirm pin";
const String kConfirmMPin = "Confirm Secure Pin";
const String kEnterConfirmMPin = "Enter Confirm Secure Pin";
const String kConfirmMPinNotMatched = "Confirm Secure pin dosn't matched";
const String kFirstSelectRoute = "First Select Route";

 ///Pharmacy Delivery List Screen
const String kDeliveryList = "Delivery List";
const String kToday = 'Today';
const String kTomorrow = 'Tomorrow';
const String kSelect = "Select";



///Pharmacy Nursing Home Screen
const String kSelectNursHome = "Select Nursing Home";


/// Imp
const String kOutForDelivery = "outfordelivery";
const String kReadyForDelivery = "eadyfordelivery";
const String kFailedDelivery = "failed";
const String kReadyDelivery = "ready";
const String kSharedDriver = "shared";
const String kDedicatedDriver = "dedicated";


/// other
const String kEndRouteAt = "End route at:";
const String kNextPharmacy = "Next Pharmacy";
const String kSelectAll = "Select All";
const String kUnSelectAll = "Unselect All";
const String kWriteSign = "Write Sign";
const String kRescheduleNow = "Reschedule Now";
const String kSelectOneEndRouteOption = "Select at least one end route option";
const String kOrdersList = "Orders List";
const String kMultipleDelivery = "Multiple Delivery...";
const String kMoreDeliveryForThisAddress = "more delivery for this address. Would you like to deliver?";
const String kDriverAlreadyOutForDelivery = "Driver is already out for delivery. You can not change status.";
const String kReadAllControlledDrug = "Please read all Controlled Drug (C.D.), Fridge and Delivery Note  ";
const String kCheckCDFridgeDeliveryNote = "Check C.D., Fridge and Delivery note";
const String kUploadingOfflineMsg = "Please wait\nWe are updating your offline deliveries.";
const String kUpdatingAndEndingRouteMsg = "Please wait\nWe are updating deliveries and ending route.";
const String kAddressDifferentWouldYouStillLikeToDeliver = "Address is different  !!!\nWould you still like to deliver?";
const String kExampleNotificationText = "Example app will continue to receive your location even when you aren't using it";
const String kRunningInBackground = "Running in Background";

/// Driver Delivery Schedule Screen
const String kDeliverySchedule = "Delivery Schedule";
const String kSurgery = "Surgery";
const String kDob = "DOB";
const String kNHS = "NHS";
const String kContact = "Contact";
const String kMeds = "Meds";
const String kRxImage = "Rx Image";
const String kRxDetails = "Rx Details";
const String kSelectService = "Select Service";
const String kSelectStatus = "Select Status";
const String kSelectSubscription = "Select Subscription";
const String kAreYouSureYouWantToCancelAllPrescriptions = "Are you sure you want to cancel all prescriptions!";


const String kPlsSlctRoute = 'Please select route';
const String kPlsSlctDriver = 'Please select driver';
const String kPlsSlctNurHome = 'Please select nursing home';
const String kPlsSlctTote = 'Please select tote';
const String kPunchSomeDel = 'Punch some deliveries';