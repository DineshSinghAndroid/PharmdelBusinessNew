...
import 'VechileInspection_Report/VechileInspection_Report_Screen.dart';
...
class Profile extends StatefulWidget {
  ...
  Profile({this.versionCode, CurrentRemainingTimeIS});
...
class ProfileState extends State<Profile> implements AuthStateListener {
  BuildContext _ctx;
  ...
  void init() async {
    ...
    await SharedPreferences.getInstance().then((value) async {
      ...
      endMile = value.getString(WebConstant.END_MILES) ?? "";
      driverType = value.getString(WebConstant.DRIVER_TYPE) ?? "";
      showPopUp = value.getBool(WebConstant.SHOW_POPUP) ?? false;
    ...
    vehicleId = prefs.getString(WebConstant.VEHICLE_ID) ?? "";
    print("vehicleId is $vehicleId");
    _username = prefs.getString('name') ?? null;
    token = prefs.getString('token') ?? "";
    userType = prefs.getString(WebConstant.USER_TYPE) ?? "";

    driverType = prefs.getString(WebConstant.DRIVER_TYPE) ?? "";
  ...
  void versionCode1() async {
    final prefs = await SharedPreferences.getInstance();

    setState(() {

      virPop = prefs.getString(WebConstant.VIR_Value);
                ...
                SliverFillRemaining(
                  ...
                  child: Padding(
                    ...
                    child: Column(children: [
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              //       onBreak = b;
                              //       updateBreakTime(lat: lat.toString(),
                              //         lng: lng.toString(),statusCodee: onBreak ==true ? 1 : 0).then((value) {
                              //         // setState(() => onBreak = b
                              //         //
                              //         // );
                              //       });
                              ...
                              //               LunchBreakPage(),)),) : _showSnackBar("Something went wrong");
                              //
                              //
                              //     },
                              //     colorBuilder: (b) => b ? Colors.red : Colors.green,
                              //     iconBuilder: (value) => value
                              //         ? Icon(Icons.emoji_food_beverage_outlined,color: Colors.white,)
                              //         : Icon(Icons.directions_bike,color: Colors.white,),
                              //     textBuilder: (value) => value
                              //         ? Center(child: Text('On Break'))
                                          ...
                                          updateBreakTime(
                                            ...
                                            SharedPreferences prefs =
                                                await SharedPreferences
                                                    .getInstance();
                                            prefs.setBool("UserOnBreak", true);
                      ...
                      /// create patient widget
                      if (howToOperateUrl != null && howToOperateUrl.isNotEmpty)
                        ListTile(
                          onTap: () {
                            if (howToOperateUrl != null &&
                                howToOperateUrl.isNotEmpty)
                              Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                      builder: (context) => PdfViewScreen(
                                          pdfUrl: howToOperateUrl)));
                          },
                          leading: Icon(
                            Icons.help,
                            size: 20,
                          ),
                          title: const Text('How to Operate'),
                        ),
                        ...
                        ListTile(
                          onTap: () async {
                            ...
                            await SharedPreferences.getInstance().then((value) {
                              startMile =
                                  value.getString(WebConstant.START_MILES) ??
                                      "";
                              vehicleId =
                                  value.getString(WebConstant.VEHICLE_ID) ?? "";
                              endMile =
                                  value.getString(WebConstant.END_MILES) ?? "";
                            });

                            if (showPopUp &&
                                (startMile == null || startMile.isEmpty)) {
                              showStartMilesDialog();
                            } else if (showPopUp &&
                                (endMile == null || endMile.isEmpty)) {
                              showEndMilesDialog();
                            } else {
                              showSimpleDialog();
                            }
                          ...
                          title: const Text('Enter Miles'),
                        ),
                      if (userType == 'Driver' &&
                          virPop == "1" &&
                          vehicleId.toString() != '0')
                        ListTile(
                          onTap: () {
                            ...
                            Navigator.push(
                                ...
                                MaterialPageRoute(
                                  builder: (context) =>
                                      VechileInspectionReportScreen(vehicleId),
                                ));
                            // isTap = true;
                            // await SharedPreferences.getInstance().then((value) {
                            //   startMile = value.getString(WebConstant.START_MILES) ?? "";
                            ...
                            // });
                            //
                            // if (showPopUp && (startMile == null || startMile.isEmpty)) {
                            //   showStartMilesDialog();
                            // } else if (showPopUp && (endMile == null || endMile.isEmpty)) {
                            //   showEndMilesDialog();
                            // } else {
                            //   showSimpleDialog();
                            // }
                      ...
                      ListTile(
                        onTap: () async {
                          ...
                          bool checkInternet =
                              await ConnectionValidator().check();
                          if (!checkInternet) {
                            noInternetPopUp(Constants.internet_notavailable);
                          } else {
                            if (!showPopUp ||
                                (endMile != null && endMile.isNotEmpty)) {
                        ...
                        title: const Text(WebConstant.LOGOUT),
                      ),
                      Spacer(),
  ...
  Future<Map<String, Object>> updateBreakTime(
      {String lat, String lng, int statusCodee}) async {
