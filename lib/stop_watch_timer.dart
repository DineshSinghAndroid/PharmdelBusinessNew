...
class StopWatchTimer {
  ...
  @Deprecated('Use functions of onStarTimer, onStopTimer, onResetTimer, onAddLap instead. '
  ...
  static int getMinute(int milliSecond) => (milliSecond / (60 * 1000) % 60).floor();
  ...
  Future<void> dispose() async {
    if (_elapsedTime.isClosed) {
      throw Exception('This instance is already disposed. Please re-create StopWatchTimer instance.');
    }
  ...
  /// Get display millisecond time.
  void setPresetHoursTime(int value, {bool add = true}) => setPresetTime(mSec: value * 3600 * 1000, add: add);

  void setPresetMinuteTime(int value, {bool add = true}) => setPresetTime(mSec: value * 60 * 1000, add: add);

  void setPresetSecondTime(int value, {bool add = true}) => setPresetTime(mSec: value * 1000, add: add);
  ...
  void clearPresetTime() {
    if (mode == StopWatchMode.countUp) {
      ...
      _presetTime = _initialPresetTime;
      _elapsedTime.add(isRunning ? _getCountDownTime(_presetTime) : _presetTime);
    } else {
  ...
  int _getCountUpTime(int presetTime) => DateTime.now().millisecondsSinceEpoch - _startTime + _stopTime + presetTime;

  int _getCountDownTime(int presetTime) => max(
        presetTime - (DateTime.now().millisecondsSinceEpoch - _startTime + _stopTime),
        0,
      );
